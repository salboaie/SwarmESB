...
exports.init = function(sf){
    ...
    sf.logger.addCase("logWhy", sendWhyLogsToParentProcess);


    sf.logger.addCase("recordTestResult", function (message, error){
        var toBeRecorded = {
            "message":message
        };
        if(error !== undefined && error.stack){
            toBeRecorded["stack"] = error.stack;
        }


        if(process['send']){
            process.send(toBeRecorded);
        }
...
function sendWhyLogsToParentProcess(logs) {
    if(process.hasOwnProperty('send')) {
        process.send({'whyLog':logs});
    }
}